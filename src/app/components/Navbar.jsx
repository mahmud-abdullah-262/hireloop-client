      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          <div className="bg-[#1A1A1A] border border-zinc-800/80 rounded-md px-6 py-2 flex items-center space-x-6 text-sm font-medium text-zinc-300">
            {navLinks.map(link=> <Link key={link.label} href={link.href} className="hover:text-white transition-colors">{link.label}</Link>)}
          </div>
...
{isOpen && (
  <div className="md:hidden mt-4 bg-[#1A1A1A] border border-zinc-800 rounded-2xl p-4 flex flex-col space-y-4 text-sm font-medium text-zinc-300">
         {navLinks.map(link=> <Link key={link.label} href={link.href} onClick={() => setIsOpen(false)} className="hover:text-white py-1 border-b border-zinc-800/50">{link.label}</Link>)}
    <div className="flex items-center space-x-4 text-sm font-medium">
      {user ? 
        <>
          <Link 
